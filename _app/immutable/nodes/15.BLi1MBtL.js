...
];`});var C=e(j,2),I=s(C),S=s(I),P=e(s(S),2),T=e(s(P));q(T,{name:"required_trust_defaults_base"}),t(P),t(S),t(I);var E=e(I);fe(E,20,()=>[null].concat(Ee),U=>U,(U,h)=>{var W=Ue(),F=s(W),re=s(F);{let f=Ae(()=>ie(h));L(re,{get content(){return Q(f)}})}t(F);var J=e(F);{var se=f=>{var M=Le();r(f,M)},be=f=>{var M=A(),ge=m(M);{var ke=N=>{var G=Re();r(N,G)},we=N=>{var G=A(),xe=m(G);{var Se=O=>{var K=Be();r(O,K)},Ce=O=>{var K=A(),Pe=m(K);{var Te=ae=>{var qe=De();r(ae,qe)};z(Pe,ae=>{h==="high"&&ae(Te)},!0)}r(O,K)};z(xe,O=>{h==="medium"?O(Se):O(Ce,!1)},!0)}r(N,G)};z(ge,N=>{h==="low"?N(ke):N(we,!1)},!0)}r(f,M)};z(J,f=>{h===null?f(se):f(be,!1)})}var le=e(J),ne=s(le),ye=s(ne,!0);t(ne),t(le),t(W),Z(f=>R(ye,f),[()=>Object.entries(Fe).filter(([f,M])=>M===h).map(([f])=>ie(f)).join(", ")]),r(U,W)}),t(E),t(C),b(2),r(g,u)},$$slots:{default:!0}});var i=e(o,2);V(i,{children:(g,H)=>{var u=Ye(),_=m(u);X(_,{text:"Explicit directives"});var l=e(_,4);L(l,{content:`export const my_csp_trusted_sources: Array<Csp_Source_Spec> = [
