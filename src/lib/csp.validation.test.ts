...
test('parse_csp_directive: accepts valid directive names', () => {
	...
	assert.strictEqual(parse_csp_directive('upgrade-insecure-requests'), 'upgrade-insecure-requests');
});
...
test('immutability: structuredClone works for complex directives', () => {
	...
	if (typeof complex_value[2] === 'object') {
		(complex_value[2] as any).nested = 'modified';
	}
	...
	assert.strictEqual(
		csp1['script-src']!.length,
		3,
		'csp1 should have original length',
	);
	assert.ok(
...
test('directive specs: all specs have valid structure', () => {
	for (const spec of csp_directive_specs) {
		...
		assert.ok(
			Array.isArray(spec.fallback_of) || spec.fallback_of === null,
...
test('directive specs: csp_directive_spec_by_name Map is correctly populated', () => {
	...
	for (const spec of csp_directive_specs) {
		assert.ok(
			csp_directive_spec_by_name.has(spec.name),
			`Map should contain ${spec.name}`,
		);
		assert.strictEqual(
