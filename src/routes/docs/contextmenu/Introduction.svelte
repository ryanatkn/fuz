...
<Tome_Section>
	...
	<p>
		This simple hierarchical pattern makes the UX have full contextual actions by default, not just
		for the target being clicked, but all ancestor behaviors too. This means users don't need to
		hunt for specific parent elements to find the desired action like in many systems -- instead all
		actions in the tree are made available, at the cost of verbosity. Devs can choose the more
		common granular and restrictive style if they wish by simply not nesting <code>Contextmenu</code
		> declarations.
	</p>
