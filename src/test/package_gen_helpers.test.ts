...
describe('package_gen_generate_ts', () => {
	test('generates valid TypeScript with correct structure', () => {
		...
		// Check eslint-disable for no-useless-escape (needed for json_embed with `\"` escapes)
		assert.ok(result.includes('/* eslint-disable no-useless-escape */'));

		// Check imports
		assert.ok(result.includes('import type {PackageJson}'));
		assert.ok(result.includes('import type {SrcJson}'));

		// Check exports use JSON.parse (json_embed strategy)
		assert.ok(result.includes('export const package_json: PackageJson = JSON.parse('));
		assert.ok(result.includes('export const src_json: SrcJson = JSON.parse('));
